...
public class AudioRecorderUtils {
	...
	private static AudioRecorderUtils result = null;

	public static AudioRecorderUtils getInstanse(Boolean recordingCompressed) {
		if (recordingCompressed) {
			result = new AudioRecorderUtils(false, AudioSource.MIC, sampleRates[3],
					AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT);
		} else {
			int i = 3;
			do {
				result = new AudioRecorderUtils(true, AudioSource.MIC, sampleRates[i],
						AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.ENCODING_PCM_16BIT);

			} while ((++i < sampleRates.length) & !(result.getState() == AudioRecorderUtils.State.INITIALIZING));
		}
		return result;
	}
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
		public void onPeriodicNotification(AudioRecord recorder) {
			...
			try {
				...
				Log.e(AudioRecorderUtils.class.getName(), "Error occured in updateListener, recording is aborted");
				// stop();
			}
		}
	 ...
	 * 
	 */
	public AudioRecorderUtils(boolean uncompressed, int audioSource, int sampleRate, int channelConfig, int audioFormat) {
		try {
			...
			if (rUncompressed) { // RECORDING_UNCOMPRESSED
				...
				if (bufferSize < AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)) { // Check
					...
					Log.w(AudioRecorderUtils.class.getName(), "Increasing buffer size to " + Integer.toString(bufferSize));
				}
			...
			if (e.getMessage() != null) {
				Log.e(AudioRecorderUtils.class.getName(), e.getMessage());
			} else {
				Log.e(AudioRecorderUtils.class.getName(), "Unknown error occured while initializing recording");
			}
			state = State.ERROR;
		}
	}
	...
	public void setOutputFile(String argPath) {
		try {
			...
			if (e.getMessage() != null) {
				...
				Log.e(AudioRecorderUtils.class.getName(), "Unknown error occured while setting output path");
			}
			state = State.ERROR;
		}
	}
	...
	public void prepare() {
		try {
			if (state == State.INITIALIZING) {
				if (rUncompressed) {
					if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null)) {
						...
						Log.e(AudioRecorderUtils.class.getName(), "prepare() method called on uninitialized recorder");
						state = State.ERROR;
					}
				} else {
				...
				Log.e(AudioRecorderUtils.class.getName(), "prepare() method called on illegal state");
				release();
				state = State.ERROR;
			}
		} catch (Exception e) {
			if (e.getMessage() != null) {
				...
				Log.e(AudioRecorderUtils.class.getName(), "Unknown error occured in prepare()");
			}
			state = State.ERROR;
		}
	}
		...
		if (state == State.RECORDING) {
			...
			if ((state == State.READY) & (rUncompressed)) {
				try {
					...
					Log.e(AudioRecorderUtils.class.getName(), "I/O exception occured while closing output file");
				}
	...
	public void start() {
		if (state == State.READY) {
			...
			Log.e(AudioRecorderUtils.class.getName(), "start() called on illegal state");
			state = State.ERROR;
		}
	}
	...
	public void stop() {
		if (state == State.RECORDING) {
			...
			Log.e(AudioRecorderUtils.class.getName(), "stop() called on illegal state");
			state = State.ERROR;
		}
	}
