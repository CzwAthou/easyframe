...
public class AppShortCutUtil {
     ...
     * @return
     */
    private static Bitmap generatorNumIcon(Context context, Bitmap icon, boolean isShowNum, String num) {
        ...
        if (isShowNum) {
            ...
            if (numInt > 99) {// 超过99
                ...
                RectF oval = new RectF(iconSize - circleRadius - textWidth + (10 * factor), 2 * factor, iconSize
                        - circleRadius, circleRadius * 2 + 2 * factor);
     ...
     * @return
     */
    private static Bitmap generatorNumIcon2(Context context, Bitmap icon, boolean isShowNum, String num) {
     ...
     * @return
     */
    private static Bitmap generatorNumIcon3(Context context, Bitmap icon, boolean isShowNum, String num) {
        ...
        if (isShowNum) {
            ...
            // 绘制数字
            canvas.drawText(num, (float) (iconSize - (backgroundWidth + textWidth + 4 * strokeThickness) / 2), (22)
                    * factor + 2 * strokeThickness, numPaint);
        }
        return numIcon;
    }
     ...
     * @return
     */
    private static Bitmap generatorNumIcon4(Context context, Bitmap icon, boolean isShowNum, String num) {
        ...
        if (isShowNum) {
            ...
            ShapeDrawable drawable = getDefaultStrokeDrawable(context);
            drawable.setIntrinsicHeight(backgroundHeight);
            drawable.setIntrinsicWidth(backgroundWidth);
            drawable.setBounds(0, 0, backgroundWidth, backgroundHeight);
    ...
    public static void addShortcut(Activity activity, int resId) {
        ...
        //注意: ComponentName的第二个参数必须加上点号(.)，否则快捷方式无法启动相应程序  (已过时)
...
//	 * @param clazz
//	 */
//	public static void deleteShortCut(Context context, Class<?> clazz, String appName, int appIconResId) {
//		Log.e(TAG, "delShortcut....");
//
//		if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi")) {
//			// 小米
//			L.e("please use this method : installXiaoMiNumShortCut");
//		} else if (Build.MANUFACTURER.equalsIgnoreCase("samsung")) {
//			// 三星
//			installSamsungNumShortCut(context, "0");
//		} else if (Build.MANUFACTURER.equalsIgnoreCase("SONY ERICSSON")) {
//			// 索尼
//			installSonyNumShortCut(context, "0");
//		} else {
//			// 其他原生系统手机
//			// 删除显示数字的快捷方式
//			deleteRawShortCut(context, clazz, appName);
//		}
//	}
     ...
     */
    private static void deleteRawShortCut(Context context, Class<?> clazz, String appName) {
    ...
    public static void addNumShortCut(Context context, Class<?> clazz, boolean isShowNum, String num, boolean isStroke,
                                      String appName, int appIconResId) {
        Log.i(TAG, "manufacturer=" + Build.MANUFACTURER);
        if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi")) {
            // 小米
            showXiaomiUnreadNum(context, num);
        } else if (Build.MANUFACTURER.equalsIgnoreCase("samsung")) {
            // 三星
            if (isShowNum) {
                showSamsungUnreadNum(context, num);
            } else {
                showSamsungUnreadNum(context, "0");
            }
        } else if (Build.MANUFACTURER.equalsIgnoreCase("SONY ERICSSON")) {
            if (isShowNum) {
                showSonyUnreadNum(context, num);
            } else {
                showSonyUnreadNum(context, "0");
            }
        } else if (Build.MANUFACTURER.equalsIgnoreCase("HTC")) {
            // HTC 经测试不能正常创建快捷方式（待研究）
//            installRawShortCut(context, clazz, isShowNum, num, isStroke, appName, appIconResId);
        } else if (Build.MANUFACTURER.equalsIgnoreCase("HUAWEI")) {
//            if (isShowNum) {
//                showHuaWeiUnreadNum(context, num);
//            } else {
//                showHuaWeiUnreadNum(context, "0");
//            }
        } else if (Build.MANUFACTURER.equalsIgnoreCase("Genymotion")) {
            // 虚拟机 经测试不能正常创建快捷方式（待研究）
        } else {// 其他原生系统手机
//            installRawShortCut(context, clazz, isShowNum, num, isStroke, appName, appIconResId);
        }
    }

    private static void showXiaomiUnreadNum(Context context, String num) {
        ...
        try {
            ...
            builder.setContentTitle("您有" + num + "条未读消息");
            builder.setTicker("您有" + num + "条未读消息");
            builder.setAutoCancel(true);
            builder.setSmallIcon(R.drawable.bg_message_num_shape);
            ...
            method.invoke(extraNotification, Integer.parseInt(num));
...
//            field.set(notification, miuiNotification);
            ...
            localIntent.putExtra("android.intent.extra.update_application_component_name", context.getPackageName() + "/" + getLaunchActivityName(context));
     ...
     * @param num
     */
    private static void showSonyUnreadNum(Context context, String num) {
     ...
     * @param num
     */
    private static void showSamsungUnreadNum(Context context, String num) {
        int numInt = Integer.valueOf(num);
        if (numInt > 99) {
            numInt = 99;
        }
        ...
        localIntent.putExtra("badge_count_class_name", getLaunchActivityName(context));
        context.sendBroadcast(localIntent);
    }

    private static boolean isHWSupportedBade(Context context) {
    ...
    private static void showHuaWeiUnreadNum(Context context, String num) {
        if (isHWSupportedBade(context)) {
            try {
                Bundle extra = new Bundle();
                extra.putString("package", context.getPackageName()); //package:应用包名
                extra.putString("class", getLaunchActivityName(context)); // class:桌面图标对应的应用入口 Activity 类
                extra.putInt("badgenumber", Integer.parseInt(num)); //badgenumber:角标数字(必须为大于或等于 0 的数字，为 0 时不显示角标)。
     ...
     * @param isStroke  是否加上边框
     */
    private static void installRawShortCut(Context context, Class<?> clazz, boolean isShowNum, String num,
                                           boolean isStroke, String appName, int appIconResId) {
        Log.i(TAG, "installShortCut....");
        ...
        if (isStroke) {
            shortcutIntent.putExtra(
                    ...
                    generatorNumIcon4(context,
                            ((BitmapDrawable) context.getResources().getDrawable(appIconResId)).getBitmap(), isShowNum,
                            num));
        } else {
            shortcutIntent.putExtra(
                    Intent.EXTRA_SHORTCUT_ICON,
...
//     *
//     * @param context
//     * @param number  显示的数字：大于99，为"99"，当为"0"时，不显示数字，相当于隐藏了)<br>
//     *                <br>
//     *                <p>
...
//    public static boolean installXiaoMiNumShortCut(Context context, int noticeId, Notification notification, int number) {
//        L.e("xiaoMiShortCut....");
//
...
//    }
    ...
    public static String getLaunchActivityName(Context context) {
        ...
        try {
            ...
            while (localIterator.hasNext()) {
                ...
                if (!localResolveInfo.activityInfo.applicationInfo.packageName.equalsIgnoreCase(context
                        .getPackageName()))
                    continue;
	 ...
	 * 
	 * //这个是为了应对不同分辨率的手机，屏幕兼容性 int r =
	 * dipToPixels(context,DEFAULT_CORNER_RADIUS_DIP); float[] outerR = new
	 * float[] {r, r, r, r, r, r, r, r}; int distance =
	 * dipToPixels(context,DEFAULT_STROKE_WIDTH_DIP);
	 * 
	 * //圆角矩形 RoundRectShape rr = new RoundRectShape(outerR, null, null);
	 ...
	 * drawable.getStrokepaint().setStrokeWidth(distance);//设置边框宽度 return
	 * drawable;
	 * 
	 * }
	 */
