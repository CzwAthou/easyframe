...
public final class AbStrUtil {
    ...
    public static boolean isPassport(@NonNull String passport) { //验证护照   eg:E37277459
    ...
    /**
     * 身份证第18位（校验码）的计算方法  420222198911098314
     ...
     * @return
     */
    public static String join(Object[] arrys, CharSequence delimiter) {
        if (arrys == null) {
            return "";
        }
        StringBuilder sb = new StringBuilder();
     ...
     * @return
     */
    public static String join(List<String> arrys, CharSequence delimiter) {
        if (arrys == null) {
            return "";
        }
        StringBuilder sb = new StringBuilder();
        for (String token : arrys) {
            if (sb.length() > 0) {
                sb.append(delimiter);
            }
            sb.append(token);
    ...
    public static String reverseSentence(String source) {
        ...
        String[] words = source.trim().split(" +");//' +'表示匹配多个空格
        Collections.reverse(Arrays.asList(words));
        return join(words, " ");
    }
