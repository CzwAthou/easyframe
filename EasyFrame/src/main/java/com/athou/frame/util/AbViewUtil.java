...
public class AbViewUtil {
    ...
    private void setDialogLayoutParams(Dialog dialog, int dialogPadding, int gravity) {
        ...
        // 设置宽度
        lp.width = AbAppUtil.getScreenWidth() - dialogPadding;
    ...
    public static int scaleValue(Context context, float value) {
        DisplayMetrics mDisplayMetrics = AbAppUtil.getDisplayMetrics();
        // 为了兼容尺寸小密度大的情况
        if (mDisplayMetrics.scaledDensity > AbAppData.UI_DENSITY) {
            // 密度
            if (mDisplayMetrics.widthPixels > AbAppData.UI_WIDTH) {
                value = value * (1.3f - 1.0f / mDisplayMetrics.scaledDensity);
            } else if (mDisplayMetrics.widthPixels < AbAppData.UI_WIDTH) {
    ...
    public static int scale(int displayWidth, int displayHeight, float pxValue) {
        ...
        float scale = 1;
        try {
            float scaleWidth = (float) displayWidth / AbAppData.UI_WIDTH;
            float scaleHeight = (float) displayHeight / AbAppData.UI_HEIGHT;
